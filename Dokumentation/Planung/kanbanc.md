...
| B0032 | Bubble | F0007 | Spaltenpflege in der Oberflaeche | gelb | Test gruen | Board-Seite → Board.razor bindet Spaltenpflege.razor ein → Spalte angelegt und geaendert, Meldung bei Zurueckweisung | 2 |  |  |  | UI; die Pflege sitzt unter den Spaltenbahnen auf /boards/{BoardId}, nicht mehr im abgebauten Detail-Panel von Boards.razor |
| B0033 | Bubble | F0007 | E2E Spalte anlegen und aendern | gelb | Test gruen | beide Prozesse auf freien Ports → Playwright → US-1, US-2, US-6, US-7, US-8 | 2-4 |  |  |  | Die Spalten-Locator liegen im Seitenobjekt BoardSeite; BoardsSeite fuehrt sie nicht mehr |
| B0041 | Bubble | F0007 | Ausfall der WebApi in der Spaltenpflege | gelb | Test gruen | HttpRequestException → WebApiAufruf.MitAusfallmeldung → Ausfallmeldung statt Absturz | 0,4 |  |  |  | Operation; Klammer aus Boards.razor hochgezogen, zweiter Nutzer (C23) |
...
| B0037 | Bubble | F0008 | Umsortieren in der Oberflaeche | gelb | Test gruen | Hoch/Runter → Spaltenpflege.razor + SpaltenApiKlient → neue Ordnung; E2E US-3 | 2 |  |  |  | UI; sendet die ganze Reihenfolge, nicht eine Einzelposition |
...
| B0040 | Bubble | F0009 | Entfernen in der Oberflaeche | gelb | Test gruen | Entfernen → Spaltenpflege.razor + SpaltenApiKlient → Spalte verschwindet; E2E US-4, US-5 | 2 |  |  |  | UI |
