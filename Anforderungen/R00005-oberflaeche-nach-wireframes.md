...
**Zwei abgelöste Zusagen aus der Gestaltungsarbeit nach der Umsetzung.** Beim Durchsehen des laufenden Stands wurden zwei Kriterien dieser Anforderung bewusst abgelöst. Sie bleiben abgehakt, weil sie zum Zeitpunkt der Abnahme erfüllt waren; was heute gilt, steht hier.

- **Die Wortmarke entfällt.** Das Kriterium unter „Rahmen und Navigation" lautet: *„Der Rahmen zeigt eine waagerechte Kopfzeile mit der Marke `KanbanC` links und den Navigationspunkten `Boards`, `Auswertungen`, `Kontributoren`."* An der Stelle der Marke steht jetzt der Titel der geöffneten Seite — auf der Übersicht „Boards", auf einem Board dessen Name. Der Grund: Wer die Anwendung offen hat, weiß, welche es ist; der Platz in der Zeile gehört dem Inhalt. Die drei Navigationspunkte bleiben, aber `Boards` wird ausgeblendet, solange ein Board offen ist — der Rückweg in derselben Zeile führt schon dorthin. `RahmenE2ETests` prüft entsprechend den Seitentitel statt der Marke.
- **Der Vermerk der Abschlussspalte ist kürzer.** Das Kriterium unter „Schirm Board" verlangt, dass die Abschlussspalte *„an ihrer Bezeichnung mit Häkchen erkennbar ist und ihre Anzeigegrenze nennt"* — beides gilt unverändert. Der Wortlaut ist von „Abschlussspalte, Anzeigegrenze 20" auf „Grenze 20" gekürzt, damit der Kopf einzeilig bleibt und die Trennlinie in jeder Bahn auf derselben Höhe sitzt. Die Bedeutung „Abschlussspalte" trägt seither das Häkchen in seinem `title`.

**Eine technische Änderung, die keine Zusage berührt, aber Folgen hat.** Der Rendermode ist von den einzelnen Seiten an die Wurzel gewandert (`App.razor`). Vorher war das Layout statisch und nur `Board` und `Boards` interaktiv; `SectionOutlet` in der Kopfzeile und `SectionContent` in der Seite erreichen einander über diese Grenze nicht. Seither ist die ganze Anwendung interaktiv. Für eine LAN-Anwendung ohne Weiteres tragbar — wer später über die Last nachdenkt, findet hier den Anlass.
